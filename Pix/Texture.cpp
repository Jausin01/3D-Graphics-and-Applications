...
#pragma pack(pop)
    ...
    X::Color GetBilinearFilterPixelColor(const Texture& tex, float u, float v)
    {
        ...
        // step 2, convert float to ints to get piel indices
...
void Texture::Load(const std::string& fileName)
{
    ...
    {
        char buffer[128];
        sprintf_s(buffer, "Can't open the file %s", fileName.c_str());
    ...
    {
        fclose(file);
        MessageBoxA(nullptr, "File not 24 bit aligned!", "Texutre Error!", MB_OK | MB_ICONEXCLAMATION);
        return;
    }
    ...
    for (int h = 0; h < mHeight; ++h)
    {
        for (int w = 0; w < mWidth; ++w)
        {
            ...
            mPixels[index] = { r / 255.0f, g / 255.0f, b / 255.0f, 1 };
